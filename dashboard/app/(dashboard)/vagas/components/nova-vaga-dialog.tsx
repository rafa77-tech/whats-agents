...
export function NovaVagaDialog({ open, onOpenChange, onSuccess }: NovaVagaDialogProps) {
  ...
  // Refs to track the latest search values inside cmdk callbacks
  ...
  const canSubmit = (): boolean => {
    ...
    if (!especialidadeId) return false
    if (!data) return false
    if (!contatoNome || contatoNome.trim().length < 2) return false
    if (!contatoWhatsapp || !/^\d{10,13}$/.test(contatoWhatsapp)) return false
    return true
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (horaFim) payload.hora_fim = horaFim
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label>
              Contato Responsavel <span className="text-destructive">*</span>
            </Label>
            ...
            {/* WhatsApp field - show when manual or when selected contato has no phone */}
            {contatoNome && (contatoManual || !contatoWhatsapp) && (
              <div className="space-y-2 pt-1">
                <Label>
                  WhatsApp do contato <span className="text-destructive">*</span>
                </Label>
                <Input
          ...
          <div className="space-y-2">
            ...
            <Textarea
              placeholder="Ex: Plantao noturno, necessario experiencia em UTI"
              value={observacoes}
