...
export default function DashboardPage() {
  ...
  const [operationalData, setOperationalData] = useState<OperationalStatusData>(defaultOperationalStatus);
  ...
  const handleExport = async (format: "csv" | "pdf") => {
    try {
      const response = await fetch(`/api/dashboard/export?format=${format}&period=${selectedPeriod}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-[1600px] mx-auto p-6 space-y-6">
        ...
        <section aria-label="Pool de Chips" className="space-y-6">
          {chipPoolData ? (
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <p className="text-gray-500 text-center">Carregando pool de chips...</p>
            </div>
          )}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            {chipsList.length > 0 ? (
              ...
              <p className="text-gray-500 text-center">Nenhum chip encontrado</p>
            )}
          </div>
        </section>
        ...
        <section aria-label="Tendencias e Alertas" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {trendsData.metrics.length > 0 ? (
            <TrendsSection data={trendsData} />
          ) : (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <p className="text-gray-500 text-center">Sem dados de tendencias</p>
            </div>
          )}
