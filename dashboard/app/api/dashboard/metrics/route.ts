...
export async function GET(request: NextRequest) {
  try {
    ...
    const conversionCurrent = calculateRate(
      fechamentosCurrent || 0,
      respostasCurrent || 0
    );
    const conversionPrevious = calculateRate(
      fechamentosPrevious || 0,
      respostasPrevious || 0
    );

    // Normalizar fechamentos por semana
    const weeksInPeriod = days / 7;
    const closingsPerWeekCurrent = Number(
      ((fechamentosCurrent || 0) / weeksInPeriod).toFixed(1)
    );
    const closingsPerWeekPrevious = Number(
      ((fechamentosPrevious || 0) / weeksInPeriod).toFixed(1)
    );
