...
export async function GET(request: NextRequest) {
  try {
    ...
    typedChips.forEach((chip) => {
      ...
      const level =
        (chip.trust_level as TrustLevel) || getTrustLevel(chip.trust_score || 0);
      trustCounts[level]++;
    });

    const totalChips = typedChips.length || 1;
    const trustDistribution = (
      Object.entries(trustCounts) as [TrustLevel, number][]
    ).map(([level, count]) => ({
      level,
      count,
    ...
    const avgBlockRate =
      activeChips.length > 0
        ...
        : 0;
    const totalErrors =
      typedChips.reduce((sum, c) => sum + (c.erros_ultimas_24h || 0), 0) || 0;
    ...
    if (previousMetrics && previousMetrics.length > 0) {
      ...
      previousErrors = previousMetrics.reduce(
        (sum, m) => sum + ((m.erros as number) || 0),
        0
      );
    } else {
