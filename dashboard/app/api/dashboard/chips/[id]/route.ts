...
export async function GET(
  ...
  try {
    ...
    const { data: transitions } = await supabase
      ...
      .select(
        "from_status, to_status, from_trust_score, to_trust_score, triggered_by, created_at"
      )
      .eq("chip_id", chipId)
      .order("created_at", { ascending: false })
      .limit(10);
    ...
    // Formatar resposta
    const trustLevel =
      (typedChip.trust_level as TrustLevel) ||
      getTrustLevel(typedChip.trust_score || 0);
    ...
    return NextResponse.json({
      chip: {
        ...
        dailyLimit: getDailyLimit(
          typedChip.status,
          typedChip.fase_warmup,
          typedChip.limite_dia
        ),
