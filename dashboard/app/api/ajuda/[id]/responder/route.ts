...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!resposta?.trim()) {
      return NextResponse.json({ detail: 'Resposta e obrigatoria' }, { status: 400 })
    }
    ...
    if (pedidoError || !pedido) {
      return NextResponse.json({ detail: 'Pedido nao encontrado' }, { status: 404 })
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ detail: 'Erro ao atualizar pedido' }, { status: 500 })
    }
