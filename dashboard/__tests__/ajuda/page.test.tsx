...
describe("CanalAjudaPage", () => {
  ...
  it("renders page title and description", () => {
    ...
    expect(
      screen.getByText("Perguntas que Julia nao soube responder")
    ).toBeInTheDocument();
  });
  ...
  it("shows empty state when no pending requests", async () => {
    ...
    expect(
      await screen.findByText("Tudo em dia!", {}, { timeout: 10000 })
    ).toBeInTheDocument();

    expect(
      screen.getByText("Nenhum pedido pendente no momento.")
    ).toBeInTheDocument();
  });
  ...
  it("renders pending requests", async () => {
    ...
    expect(
      await screen.findByText("Dr. Carlos", {}, { timeout: 10000 })
    ).toBeInTheDocument();
  ...
  it("shows alert when there are pending requests", async () => {
    ...
    expect(
      await screen.findByText(
        "1 pedido(s) aguardando resposta",
        {},
        { timeout: 10000 }
      )
    ).toBeInTheDocument();
  ...
  it("shows Responder button for pending requests", async () => {
    ...
    expect(
      await screen.findByText("Responder", {}, { timeout: 10000 })
    ).toBeInTheDocument();
  });
  ...
  it("shows response form when clicking Responder", async () => {
    ...
    const responderButton = await screen.findByText(
      "Responder",
      {},
      { timeout: 10000 }
    );
  ...
  it("shows Atualizar button", async () => {
    ...
    expect(
      await screen.findByText("Atualizar", {}, { timeout: 5000 })
    ).toBeInTheDocument();
  });
  ...
  it("shows sound toggle", async () => {
    ...
    expect(
      await screen.findByText("Som", {}, { timeout: 5000 })
    ).toBeInTheDocument();
  ...
  it("shows responded request with answer", async () => {
    ...
    expect(
      await screen.findByText(
        "Sim, o hospital tem estacionamento proprio.",
        {},
        { timeout: 10000 }
      )
    ).toBeInTheDocument();
