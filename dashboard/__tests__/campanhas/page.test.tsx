...
describe("CampanhasPage", () => {
  ...
  it("renderiza titulo e descricao da pagina", () => {
    ...
    expect(
      screen.getByText("Gerencie campanhas de prospecção e reativacao")
    ).toBeInTheDocument();
  });
  ...
  it("mostra botao Nova Campanha", async () => {
    ...
    expect(
      await screen.findByText("Nova Campanha", {}, { timeout: 5000 })
    ).toBeInTheDocument();
  });
  ...
  it("mostra cards de metricas", async () => {
    ...
    expect(
      await screen.findByText("Campanhas Ativas", {}, { timeout: 10000 })
    ).toBeInTheDocument();
  ...
  it("abre wizard ao clicar em Nova Campanha", async () => {
    ...
    const novaButton = await screen.findByText(
      "Nova Campanha",
      {},
      { timeout: 5000 }
    );
  ...
  it("mostra status correto da campanha", async () => {
    ...
    expect(
      await screen.findByText("Agendada", {}, { timeout: 10000 })
    ).toBeInTheDocument();
  });
});
