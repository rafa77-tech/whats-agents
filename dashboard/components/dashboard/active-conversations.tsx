  ...
  useEffect(() => {
    async function fetchConversations() {
      ...
      try {
        ...
        const response = await fetch(`${apiUrl}/dashboard/conversations?status=active&per_page=5`, {
          headers: {
            ...
            {conversations.map((conv) => (
              <Link key={conv.id} href={`/conversas/${conv.id}`} className="block">
                <div className="flex items-start gap-3 rounded-lg p-3 transition-colors hover:bg-accent">
                  {/* Status indicator */}
                  <span className={`mt-1.5 h-2 w-2 rounded-full ${getStatusColor(conv)}`} />

                  <div className="min-w-0 flex-1">
                    {/* Header */}
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate font-medium">{conv.cliente_nome}</p>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <span className={getStatusTextColor(conv)}>{getStatusLabel(conv)}</span>
                      {conv.unread_count > 0 && (
                        <>
                          <span>|</span>
