...
async def incrementar_tentativa(dlq_id: str, erro: str) -> int:
    ...
    """
    # Buscar atual
    result = supabase.table("webhook_dlq").select("tentativa").eq("id", dlq_id).single().execute()
...
async def obter_estatisticas() -> Dict:
    ...
    """
    supabase.table("webhook_dlq").select("status", count="exact").execute()
    ...
    for status in ["pending", "processing", "processed", "failed"]:
        count_result = (
            supabase.table("webhook_dlq").select("id", count="exact").eq("status", status).execute()
        )
