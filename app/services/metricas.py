...
class MetricasService:
    ...
    async def registrar_mensagem(
        ...
        try:
            ...
            if origem == "medico":
                atualizacao["total_mensagens_medico"] = (
                    metricas.get("total_mensagens_medico", 0) or 0
                ) + 1
            elif origem == "ai":
                atualizacao["total_mensagens_julia"] = (
                    metricas.get("total_mensagens_julia", 0) or 0
                ) + 1
            elif origem == "humano":
                atualizacao["total_mensagens_humano"] = (
                    metricas.get("total_mensagens_humano", 0) or 0
                ) + 1
            ...
            if tempo_resposta_segundos:
                ...
                # Atualizar média
                total_respostas = (metricas.get("total_mensagens_julia", 0) or 0) + (
                    metricas.get("total_mensagens_humano", 0) or 0
                )
                if total_respostas > 0:
                    media_atual = metricas.get("tempo_medio_resposta_segundos") or 0
                    nova_media = (media_atual * total_respostas + tempo_resposta_segundos) / (
                        total_respostas + 1
                    )
            ...
            supabase.table("metricas_conversa").update(atualizacao).eq(
                "id", metricas["id"]
            ).execute()
